...
setupDWM() {
    ...
    case "$PACKAGER" in # Install pre-Requisites
        pacman)
            Package_installer_ base-devel libx11 libxinerama libxft imlib2 libxcb git unzip flameshot lxappearance feh mate-polkit meson libev uthash libconfig
            ;;
        apt-get|nala)
            Package_installer_ build-essential libx11-dev libxinerama-dev libxft-dev libimlib2-dev libx11-xcb-dev libfontconfig1 libx11-6 libxft2 libxinerama1 libxcb-res0-dev git unzip flameshot lxappearance feh mate-polkit
            ;;
        dnf)
            Package_installer_ groupinstall -y "Development Tools"
            Package_installer_ libX11-devel libXinerama-devel libXft-devel imlib2-devel libxcb-devel unzip flameshot lxappearance feh mate-polkit
            ;;
        *)
...
makeDWM() {
    ...
    my-superuser make clean install # Run make clean install
}
install_nerd_font() {
    ...
    if [ ! -f "$FONT_ZIP" ]; then
        ...
        getURL 'download2' "$FONT_URL" "$FONT_ZIP" || {
...
picom_animations() {
    ...
    if ! my-superuser ninja -C build install; then
...
setupDisplayManager() {
    ...
    case "$PACKAGER" in
        pacman)
            Package_installer_ xorg-xinit xorg-server
            ;;
        apt-get|nala)
            Package_installer_ xorg xinit
            ;;
        dnf)
            Package_installer_ xorg-x11-xinit xorg-x11-server-Xorg
            ;;
        *)
            printf "%b\n" "${RED}Unsupported package manager: $PACKAGER${RC}"
            exit 1
    ...
    if [ "$currentdm" = "none" ]; then
        ...
        case "$PACKAGER" in
            pacman)
                Package_installer_ "$DM"
                if [ "$DM" = "lightdm" ]; then
                    Package_installer_ lightdm-gtk-greeter
                fi
                ;;
            apt-get|nala)
                Package_installer_ "$DM"
...
install_slstatus() {
    ...
    if [ "$response" = "y" ] || [ "$response" = "Y" ]; then
        ...
        if my-superuser make clean install; then
